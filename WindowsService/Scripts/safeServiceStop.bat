...
:: Modified by Arseni Rynkevich

set _find=%windir%\System32\find.exe
...
:ResolveInitialState
SC %server% query %1 | %_find% "STATE" | %_find% "RUNNING" >NUL
IF errorlevel 0 IF NOT errorlevel 1 GOTO StopService
SC %server% query %1 | %_find% "STATE" | %_find% "STOPPED" >NUL
IF errorlevel 0 IF NOT errorlevel 1 GOTO StoppedService
SC %server% query %1 | %_find% "STATE" | %_find% "PAUSED" >NUL
IF errorlevel 0 IF NOT errorlevel 1 GOTO SystemOffline
echo Service State is changing, waiting for service to resolve its state before making changes
sc %server% query %1 | %_find% "STATE" >NUL
